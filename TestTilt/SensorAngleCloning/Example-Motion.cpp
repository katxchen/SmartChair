	...
	try {
		for (size_t i = 0; i < NUM_MOVES; i++)
		{
			for (size_t iNode = 0; iNode < myPort.NodeCount(); iNode++) {
				...
				if (angle != LONG_MIN) {
					double realAng = angle / 10000.0;
					double com = -realAng / 0.0037 + 500;
					double command = realAng;
					...
					//std::cout << "Res: " << com << std::endl;
					int safetyValue = 2500;

					if (com > safetyValue)
					{
						std::cout << "Command " << com << " is too big" << std::endl;
						com = safetyValue;
					}
					else if (com < -safetyValue)
					{
						std::cout << "Command " << com << " is too small" << std::endl;
						com = -safetyValue;
					}
					...
					theNode.Motion.MovePosnStart(com, true);			//Execute angle encoder count move 

					theNode.Motion.PosnMeasured.Refresh();
					theNode.Motion.PosnCommanded.Refresh();
					//printf("com: %d\t\t pos: %f\t\t command: %f\t\t time: %f\n", com, theNode.Motion.PosnMeasured.Value(), theNode.Motion.PosnCommanded.Value(), time_diff);
